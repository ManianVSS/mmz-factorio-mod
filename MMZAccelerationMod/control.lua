...
--set storage["initial_chests"] to empty table if not set
if storage["initial_chests"] == nil then storage["initial_chests"] = {} end

-- set storage["_aegis_on"] to false if not set
if storage["_aegis_on"] == nil then storage["_aegis_on"] = false end

-- set storage["_spawn_position"] to 0,0 if not set
if storage["_spawn_position"] == nil then storage["_spawn_position"] = {0, 0} end

-- Call add_commands in control.lua to register the commands when the mod is loaded
add_Commands()
